...
class RobustDARTSOptimizer(MetaOptimizer):
    """
    ...
    def add_alphas(self, edge):
        ...
        if type(group) == list:
            alpha = torch.nn.ParameterList([self.groups[i] for i in group])
    ...
    def update_ops(edge):
        ...
        if mixedop == "cross_op":
            edge.data.set("op", DARTSMixedOpCross(primitives))
        else:
    ...
    def __init__(
        ...
        """
        super(RobustDARTSOptimizer, self).__init__()
        self.groups = {}
        self.config = config
    ...
    def adapt_search_space(self, search_space, scope=None, **kwargs):
        ...
        # 1. add alphas
        graph.update_edges(
            self.add_alphas, scope=scope, private_edge_data=False
        )
        ...
        for alpha in set(graph.get_all_edge_data("alpha")):
            if type(alpha) == torch.nn.ParameterList:
